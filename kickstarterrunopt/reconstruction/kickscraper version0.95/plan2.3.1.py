...
def writeafile(x,file):
    ...
    for i in range(0, lenallurl_clean_list):
        if clean_list[i] != '':
            #print clean_list[i]
...
def rewardsseperategenerateprocess(rewards):
    ...
    if lenrewards_backers_distribution<50:
        for i in range(lenrewards_backers_distribution):
...
def compareindexprocess(url):
    ...
    try:
        ...
        content = urllib.request.urlopen(someurl).read()
...
def webscraper_successed(someurl,a,the_page):
    ...
    try:
        ...
        content = urllib.request.urlopen(someurls).read()
        sel= etree.HTML(content)
        ...
        creator_bio_info = urllib.request.urlopen(creator_bio_info_url).read()
        ...
        if data_pool_url  != '':
            data_pool_url_websites = urllib.request.urlopen(data_pool_url).read()
...
y=eval(input('choose the number of workers for this jobs:'))
publicpath=eval(input('please enter the document path for saving file:'))
url_file=eval(input ('please enter the target url file for starting,need add /:'))
