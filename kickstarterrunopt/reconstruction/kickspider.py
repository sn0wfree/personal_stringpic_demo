...
def webscraper_successed(someurl):
    ...
    try:
        ...
        for word in suburl_words:
            globals()['someurl_%s'% word] = someurl + '/'+word
            globals()['response_%s'% word]= Request(globals()['someurl_%s'% word])
            globals()['content_%s'% word] = urllib2.urlopen(globals()['someurl_%s'% word]).read()
            globals()['sel_%s'% word]= etree.HTML(globals()['content_%s'% word])
            globals()['req_%s'% word] = urlopen(globals()['response_%s'% word])
            globals()['the_page1_%s'% word] = globals()['req_%s'% word].readlines()
            #print locals()['sel_%s',% word]
        #print  sel_description
        ...
        for line in the_page1:
            ...
            if 'data'  in line:
                ...
                for word in words:
                    if 'data class="Project' in word:
                        project_ID_str = word.split('Project')[1]
        for line in the_page1_comments:
            #project_ID_str
            if 'data'  in line:
                words = line.split('" ')
                for word in words:
        ...
        #location_id
        #createddate/set up date
        location_id_str = sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[1]/div[1]/div[2]/div[1]/div/div/a[1]/text()')
        location_id =''.join(location_id_str).strip('\n')
        project_ID = ''.join(project_ID_str)
        created_at=''.join(created_at_str)
        category = sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[1]/div[1]/div[2]/div[1]/div/div/a[2]/text()')
        state_changed_at=''.join(state_changed_at_str)
        deadline_quot=''.join(deadline_quot_str)
        #backers_count
        backers_count= sel.xpath('//*[@id="content-wrap"]/section/div[2]/div[2]/div/div/div[2]/div[4]/b/text()')
        #goal
        goal = sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[1]/div[1]/div[2]/div[2]/div/span/text()')
        #print goal
        #pledged_amount
        ...
        day_left = sel.xpath('//*[@id="stats"]/div/div[3]/div/div/div/text()')
        #data-duration
        data_duration = sel_description.xpath('//*[@id="project_duration_data"]//@data-duration')
        #updates
        updates = sel.xpath('//*[@id="content-wrap"]/div[2]/div/div/div/div[2]/a[3]/span/text()')
        #rewardsstructure
        #rewards
        rewards_level_divided_by_goal = sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[2]/div[1]/div/ol/li[*]/div[2]/h2/span[1]/text()')
        #print rewards_level_name
        rewards_level_name = sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[2]/div[1]/div/ol/li[*]/div[2]/div[1]/p/text()[1]')
        rewards_backers_level_distribution =sel_description.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[2]/div[1]/div/ol/li[*]/div[2]/div[3]/span[@class="pledge__backer-count"]/text()')
        #rewards_backers_level_distribution =sel.xpath('//*[@id="content-wrap"]/div[2]/section[1]/div/div/div/div/div[2]/div[1]/div/ol/li[*]/div[2]/div[3]/span[3]')
        #print rewards_level.spilt()
        #rewards_level_description
        #print len(rewards_level)
        ...
        for i in range(1,len(rewards_level_divided_by_goal)):
            ...
            #split each variable
            #rewards_level_description
            rewards_level_description_split_list = sel_description.xpath(rewards_level_description_a)
            rewards_level_description_split = rewards_level_description_split_list
            rewards_level_description_split =''.join(rewards_level_description_split)
            rewards_level_description.append(rewards_level_description_split)
            #pledge_limit
            pledge_limit_split_list = sel_description.xpath(pledge_limit_a)
            pledge_limit_split = pledge_limit_split_list
            pledge_limit_split =''.join(pledge_limit_split)
            pledge_limit.append(pledge_limit_split)
            #ship_location_info
            ship_location_info_split_list = sel_description.xpath(ship_location_info_a)
            ship_location_info_split = ship_location_info_split_list
            #print ship_location_info_split, ship_location_info_split_list
            ship_location_info_split =''.join(ship_location_info_split)
            ship_location_info_split= str(ship_location_info_split)
